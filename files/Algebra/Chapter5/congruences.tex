...
\begin{definition}[Congruences]
    Let $m(x)\in\f[x]$ with $m(x)\neq 0$.
    If $a(x),b(x)\in\f[x]$, we define
    \[
    ...
    \[
        m(x)\mid a(x)-b(x).
    \]
    Equivalently, there exists $q(x)\in\f[x]$ such that
    \[
        a(x)-b(x)=q(x)m(x).
    \]
    Equivalently,
...
\begin{definition}[Congruence Class]
    The congruence class of $a(x)\in\f[x]$ is denoted by $[a(x)]$.
    It consists of
    \[
        [a(x)] \coloneqq \{b(x)\in\f[x] : b(x)\equiv a(x)\mod m(x)\}.
    \]
\end{definition}
