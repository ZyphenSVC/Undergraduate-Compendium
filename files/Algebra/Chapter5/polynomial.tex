...
Feel free to check that $\ri[x]$ is a ring, but we will be concentrating on $\z[x],\Q[x],\rr[x],\c[x],\zp[x]$, and have their elements denoted by $f(x)$ or $P(x)$.
...
\]
    ...
    {\Huge{From now on $\ri$ is an Integral Domain.}}\\Given that $\ri$ is an integral domain, one may naturally ask what are the units of $\ri[x]$.
