...
\begin{theorem}[Division Algorithm in Polynomial Fields]{
    Suppose $\ff$ is a field and $a(x),b(x)\in\ff[x]$ with $b(x)\neq 0$.
    Then there exist unique $q(x),r(x)\in\ff[x]$ such that
    \[
        a(x)=q(x)b(x)+r(x)
    \]
    ...
    \]
    with $a_n,b_m\neq 0$ and $n\geq m$.
    Since $\ff$ is a field, $b_m^{-1}$ exists.
    Let
    \[
    ...
    \[
        \deg(h(x))<\deg(a(x)).
    \]
    By strong induction on $\deg(a(x))$, there exist $q_1(x),r(x)\in\ff[x]$ such that
    \[
        h(x)=q_1(x)b(x)+r(x)
    \]
    with $\deg(r(x))<\deg(b(x))$ or $r(x)=0$.
    ...
    \]
    If $(q_1(x)-q_2(x))b(x)\neq 0$, then since $\ff[x]$ is an integral domain,
    \[
...
\begin{definition}[Logical Divide of Polynomial Fields]
    Let $a(x),b(x)\in\ff[x]$ and $b(x)\neq 0$.
    We say $b(x)\mid a(x)$ if there exists a $q(x)\in\ff[x]$ such that
    \[
        a(x)=q(x)b(x).
    \]
\end{definition}
...
\begin{definition}[GCD of Polynomial Fields]
    Suppose $a(x),b(x)\in\ff[x]$ are not both $0$.
    We say $d(x)=\gcd(a(x),b(x))$ if $d(x)\mid a(x)$, $d(x)\mid b(x)$, and whenever $c(x)\in\ff[x]$ satisfies $c(x)\mid a(x)$ and $c(x)\mid b(x)$, then $c(x)\mid d(x)$.
\end{definition}
...
\begin{definition}[Monic]
    If $d(x)\in\ff[x]$ has leading coefficient $1$, then $d(x)$ is monic.
\end{definition}
...
\begin{definition}[Polynomial Associates]
    If $c(x),d(x)\in\ff[x]$ and
    \[
        c(x)=\beta d(x)
    \]
    for some $\beta\in\ff$ with $\beta\neq 0$, we say $c(x)$ and $d(x)$ are associates.
\end{definition}
...
\begin{theorem}[GCD Theorem]{
    ...
    Let
    \[
        S\coloneqq \{u(x)a(x)+v(x)b(x)\neq 0 : u(x),v(x)\in\ff[x]\}.
    \]
    Then there exist $u(x),v(x)\in\ff[x]$ such that
    \[
        d(x)=u(x)a(x)+v(x)b(x)
    \]
    and $d(x)=\gcd(a(x),b(x))$.
    ...
    Now since $d(x)\in S$, there exist $u(x),v(x)\in\ff[x]$ such that
    \[
        d(x)=u(x)a(x)+v(x)b(x).
    \]
    So
...
\begin{corollary}[Consequence of GCD Theorem]{
    Suppose $a(x),b(x)\in\ff[x]$ are relatively prime and $c(x)\in\ff[x]$.
...
If we let $\ri=\ff[x]$, we notice that it has very similar properties to $\z$, such that it has the division and gcd algorithm.
...
\begin{definition}[Irreducible]
    A polynomial $p(x)\in\ff[x]$ is irreducible if whenever
    \[
        p(x)=a(x)b(x)
    \]
    for $a(x),b(x)\in\ff[x]$, then $a(x)$ is an associate of $p(x)$ or $a(x)$ is a unit.
\end{definition}