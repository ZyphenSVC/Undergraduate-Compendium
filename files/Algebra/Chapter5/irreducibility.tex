...
\begin{proposition}[Polynomial Euclid's Lemma]{
    Suppose $p(x)\in\ff[x]$ is irreducible and $b(x)\in\ff[x]$ such that $p(x)\nmid b(x)$.
    Then
    \[
        \gcd(p(x),b(x))=1.
    \]
    ...
    Hence either $d(x)=1$, or $d(x)=cp(x)$ for some nonzero $c\in\ff$.
...
\begin{corollary}{
    If $p(x)\mid a_1(x)\cdots a_n(x)$, where $a_i(x)\in\ff[x]$ and $p(x)$ is irreducible, then $p(x)\mid a_i(x)$ for some $i$.
}
...
Use strong induction on $\deg(a(x))$ for existence.For uniqueness, suppose
\begin{align*}
...
\begin{proposition*}{
    If $f(x)$ is irreducible in $\ff[x]$, then so is every associate of $f(x)$.
}
\end{proposition*}
...
\begin{proof}
    We have
    \[
        x^2-2=\left(x-\sqrt{2}\right)\left(x+\sqrt{2}\right)\in\rr[x].
    \]
...
Let $\ff$ be a field.
Take $f(x)\in\ff[x]$.
There is a corresponding polynomial function $\ff\to\ff$ also denoted by $f(x)$.

\begin{theorem*}[Factor Theorem]{
    Let $f(x)\in\ff[x]$ and $a\in\ff$.
    If $f(a)=0$, then $(x-a)$ is a factor of $f(x)$.
    That is,
    \[
        f(x)=g(x)(x-a)
    \]
    for some $g(x)\in\ff[x]$.
}
\end{theorem*}
...
(b) Factor $x^4-4$ as a product of irreducible in $\mathbb{Z}_5[x]$.We have
\[
    x^4-4=(x^2+2)(x^2-2).
\]
...
\begin{theorem*}[Remainder Theorem]{
    ...
    \[
        f(x)=g(x)(x-a)+r(x)
    \]
    for some $g(x)\in\ff[x]$, where $r(x)$ is a constant polynomial.
}
\end{theorem*}
...
\begin{corollary}[of Factor Theorem]{
    Suppose $f(x)\in\ff[x]$ has $\deg f(x)=n$.
...
But let us look at an example of how to apply this.Let
\[
    f(x)=6x^2-5x+1.
\]
Then
...
\begin{proposition}{
    ...
    and
    \[
        \omega+\overline{\omega}\in\rr,\qquad \omega\overline{\omega}\in\rr.
    \]
...
\begin{example}
    ...
    By the Intermediate Value Theorem, there exists $c\in\rr$ such that $f(c)=0$.
