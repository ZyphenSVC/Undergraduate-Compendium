...
Now that we have learned the axioms of arithmetic, let us learn about the division algorithm.We have all, hopefully, learned how to divide in grade school.
...
And that is exactly what we spend the rest of the proof doing.We answer questions in this proof such as, what if $a$ is greater than $0$ or less than $0$.
...
Regardless of what $q$ and $r$ are, if they exist, then they are unique, sounding trivial, but as we see the proof is rather less trivial.This one is a bit more straightforward, therefore there will not be a conceptualizing analysis on this proof.
...
It is the logical notation for the phrase ``$x$ divides $y$'', and it is trivial to Abstract Algebra.It is slightly different from, say, previous computationally algebraic courses, where one just computes some division and may even end up with a completed or incomplete, rational or not, answer.
Note that up to now we are only sticking with the integers, and this is a really important fact to keep in mind.
Therefore when we say that $2\mid 4$, then we really mean that $4$ is evenly divisible by $2$, but $3$ does not divide $4$, even if we can write it in terms of a decimal.Another way we can explain this topic is through the division algorithm.
...
\begin{lemma}{
    ...
    Similarly, $b\mid -a$.
\end{lemma}This is just a fun fact to rationalize that these four results are possible:
...
This is more of a computational math.The gcd algorithm can be written in terms of the Division Algorithm and continuing to find the terms that make up the two factors.
...
This is a way to do the gcd algorithm through division.But what if we are to set this the other way around.
