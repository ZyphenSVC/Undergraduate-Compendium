...
Most of the course we will be looking at finite groups.

Let $X$ be a set.
Assume $X\neq \varnothing$.
Let $S_X$, called the symmetric group on $X$, be the set of permutations of $X$, equivalently the bijections $X\to X$.
...
If $f$ is a permutation of $X$, then its unique inverse is denoted by $f^{-1}$.

An abstract group $G$ is a nonempty set with a binary operation, usually denoted by $\cdot$, $*$, or juxtaposition.
