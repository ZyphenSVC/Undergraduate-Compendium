...
\begin{definition}[Field]
    A field $\ff$ is a commutative ring with $1\neq 0$ such that if $a\in\ff$ and $a\neq 0$, then $a$ is a unit.
\end{definition}

\begin{definition}[Subfield]
    If $S$ is a subring of a field $\ff$, and is also closed under multiplicative inverses of nonzero elements, then it is also a subfield.
\end{definition}
...
\end{lemma}
    ...
    {\Huge{From now on $\ri,\si$ is a ring and $\ff$ is a field.}}\\
...
\begin{corollary}{
    $\ff$ is an integral domain.
}
    Suppose $a,b\in\ff$ with $ab=0$.
    Suppose $a\neq 0$.
    ...
    So $\ff$ is an integral domain.
\end{corollary}

Let us look into something called extensions.

\begin{definition}[Field Adjoins]
    We call something an adjoin given that suppose we have $\ff=\mathbb{Q}$.
