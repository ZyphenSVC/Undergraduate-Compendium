...
\begin{definition}[Constructible Point]
    A point in $\r^2$ is constructible if its coordinates are constructible.
\end{definition}
...
\begin{definition}[New Constructible Points]
    ...
    \[
        \Q\subseteq\Q[a_1]\subseteq\Q[a_1,a_2]\subseteq\hdots\subseteq\f,
    \]
    where
    \[
        \f_k=\f_{k-1}[a_k]
...
So every constructible number lies in a field $\f$ where
\[
    [\f:\Q]=2^n
\]
for some $n$.
...
Therefore $x=\cos 20^\circ$ is not constructible, so $60^\circ$ cannot be trisected.
