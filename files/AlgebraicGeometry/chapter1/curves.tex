...
\begin{figure}[H]
    \centering
    \colorbox{white}{\includegraphics[width=\linewidth]{files/AlgebraicGeometry/images/circ1}}
...
\begin{definition}[Affine Space]
    Let $\a^n = \c^n$, which are n-tuples of complex numbers.
\end{definition}
...
If $f(x,y) = x^2 + y^2 - 1$, then $\{f = 0\}\subseteq\c^n = \a^n$, or $\{x^2 + y^2 - 1 = 0\}$

\begin{definition}[Affine Plane Curve]
    In fact $f(x_1,x_2) = x_1^2 + x_2^2 - 1$, thus $\{f = 0 : x_1,x_2\in\c\}\subseteq \a^2$, the affine plane, called an affine plane curve.
    Note that this curve is a $\r-\dim(f) = 2$, but $\c-\dim(f) = 1$
\end{definition}

We often only plot the real component of affine plane curves.
But if $\a = \c$, it is called an affine line, where $\cdim = 1$ and $\rdim = 2$, since we can create a complex number with two real numbers adjoined with $i$.This is due to the fact that if $y - x^2 = 0$, then we cannot draw a complex graph, but we can draw the real component as a parabola.

\begin{figure}[H]
    \centering
    \colorbox{white}{\includegraphics[width=\linewidth]{files/AlgebraicGeometry/images/graph}}
    \caption{Two Graphs of Real Components}
\end{figure}

\begin{definition}[Affine Algebraic Curve]
    $C$ is an affine curve defined by $\{f(x,y) = 0, x,y\in\c\}\subseteq\a^2$.
\end{definition}
...
Throughout this course we will be working with algebraically closed fields, for example $\r$ is not algebraically closed.
...
\begin{example}
    An example that $\r$ is not closed is shown when
    \[
    \{x^2 + 1 = 0 : x\in\r\} = \phi
    \]
    For any field, $\f$, there exists $\bar{\f}$ that is the smallest algebraically closed field containing $\f$ called the algebraic closure of $\f$.
    For example $\bar{\r} = \c$.
\end{example}

\begin{proposition}{
    Let $\k$ be a arbitrary field, $f,g\in\k[x,y]$ which are irreducible polynomials, meaning ti cannot be factored into non-constant. If $g$ is not divisible by $f$, then $f(x,y) = g(x,y) = 0$ by only finitely many satisfying solutions.
}
\end{proposition}
...
In fact, over $\r$ any conic can be given by either a hyperbola, parabola, or ellipse; However, this is not true over the complexes. If we have $C\cong C^{'}$ as affine plane curves, then we can write the following commutative diagram:
