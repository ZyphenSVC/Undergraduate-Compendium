...
Thus we may view
\[
    u : X \dashrightarrow \a^1.
\]

Given $u,v\in\c(X)$, we get a rational map
\[
    (u,v) : X \dashrightarrow \a^2.
\]

\begin{definition}[Rational Map]
    A map $\varphi : X \dashrightarrow Y$ is rational if and only if there exist rational functions $u,v\in\c(X)$ such that $(u,v): X \dashrightarrow \a^2$ factors through $Y$.
    \begin{figure}[H]
        \centering
...
\begin{remark}
    ...
    \[
        \a^1 \dashrightarrow C.
    \]
    That is, if $C : \{f(x,y)=0\}\subseteq \a^2$, then a rational parametrization has the form
    \[
        x = u(t), \qquad y = v(t)
    \]
...
Let the line $\ell\subseteq \a^2$.
Let $P\in \a^2$ be a point with $P\notin \ell$.
...
If $X$ is rational, then $X$ is birational to $\a^1$.
...
Hence $X$ is birational to $\a^1$, so $X$ is rational.
\end{theorem}
