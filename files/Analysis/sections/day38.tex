...
\begin{definition}[Pointwise Convergence]
    Given functions $f_n, f_0 : A\to\r$, we say $(f_n)$ converges pointwise to $f_0$ if for every $x\in A$,
    \[
        f_n(x)\to f_0(x).
    \]
\end{definition}
...
\begin{problem}{
    \[
        f_n(x) = \dfrac{x^2 + nx}{n}, \quad A = \r.
    \]
}
    \[
        \dfrac{x^2 + nx}{n} = x + \dfrac{x^2}{n} \to x.
    \]
    So convergence is pointwise on $\r$.
\end{problem}
...
\begin{problem}{
    ...
    However, on $\r$, for $x=n$,
    \[
        \abs{f_n(n) - f_0(n)} = \dfrac{n^2}{n} = n \to \infty,
    \]
    so convergence is not uniform on $\r$.
\end{problem}
