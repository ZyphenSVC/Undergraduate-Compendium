...
\begin{problem}{
    ...
    It is closed because every point of $\n$ is isolated, so $\n$ has no limit points in $\rr$ outside itself.
...
\begin{problem}{
    ...
    \begin{enumerate}[label={(\alph*)}]
	\item An open set that contains every rational number must necessarily be all of $\rr$.
    ...
    \[
        \rr\setminus\{\sqrt{2}\}.
    \]
    This set is open, contains every rational number, but is not all of $\rr$.
    ...
    \]
    Because $\q$ is dense in $\rr$, there exists $q\in\q$ such that
    \[
        q\in(x-\eps,x+\eps).
    \]
    Hence $q\in U$.
...
\begin{problem}{
    ...
    Factor out $B$:
    \[
        (A\cap B)\cup(A^c\cap B) = (A\cup A^c)\cap B = \rr\cap B = B.
    \]
...
\begin{example}[Examples of Connected Sets]
    \[
        \rr,\qquad \rr^n,\qquad [a,b].
    \]
\end{example}

\begin{problem}{
    Prove that the only sets that are both open and closed are $\rr$ and $\varnothing$.
}
    Suppose $S\subseteq \rr$ is both open and closed.
    ...
    This contradicts that $\rr$ is connected.
    Therefore the only clopen subsets of $\rr$ are $\rr$ and $\varnothing$.
\end{problem}
