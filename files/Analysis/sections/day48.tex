...
\begin{definition}[$C_\infty(M)$]
    \[
        C_\infty(M) \coloneqq \left\{f:M\to\r : \text{$f$ is continuous and } \sup_{m\in M}\abs{f(m)}<\infty\right\}.
    \]
...
\begin{definition}[Banach Space]
    ...
    Examples include $\r^n$, $\c^n$, $C_\infty(M)$, and $C^0([a,b])$ with the sup norm.
\end{definition}

\begin{definition}[Functional]
    A functional is a linear map $T:X\to\r$ or $T:X\to\c$.
\end{definition}
