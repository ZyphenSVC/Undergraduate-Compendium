...
\begin{example}
    Let $f_m : [0,1]\to\r$ be given by $f_m(x) = x^m$.
...
\begin{problem}{
    Show that
    \[
        \r \setminus A^{\circ} = \overline{\r \setminus A}.
    \]
}
    \textbf{($\subseteq$).}
    ...
    Hence every neighborhood of $x$ intersects $\r\setminus A$, so $x\in \overline{\r\setminus A}$.

    \textbf{($\supseteq$).}

    If $x\in \overline{\r\setminus A}$, then every neighborhood of $x$ intersects $\r\setminus A$.
...
\begin{example}
    Let $f$ be continuous on $\r$ such that
    \[
        \lim_{x\to\infty} f(x) = \infty
        \quad\text{and}\quad
        \lim_{x\to -\infty} f(x) = \infty.
...
\begin{example}
    Suppose $f : [a,b]\to\r$ is differentiable and $f\uptick$ is continuous.
    Show $f$ is uniformly differentiable.
\end{example}

\begin{example}
    Suppose $f : (a,b)\to\r$ is differentiable with $f\uptick(c)=0$ and $f^{\prime\prime}(c)>0$.
