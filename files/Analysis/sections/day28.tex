...
\begin{theorem*}{
    Suppose $f,g : A\to\rr$ and $c\in\rr$.
    If
    \[
...
\begin{definition}[One-Sided Limits]
    Given $A\subset\rr$, $c$ is a right limit point of $A$ if for all $\eps > 0$,
    \[
        (c, c+\eps)\cap A\neq\varnothing.
    \]
...
\begin{definition}[Continuous]
    Suppose $A\subset\rr$, $f:A\to \rr$, and $c\in A$.
...
\begin{remark}
    If $c$ is a limit point of $A$ and $c\in A$, then $f:A\to\rr$ is continuous at $c$ if and only if
    \[
        \lim_{x\to c} f(x) = f(c).
    \]
\end{remark}
...
\begin{theorem}{
    Suppose $f:A\to \rr$, $f(A)\subset B$, and $g:B\to\rr$.
...
\begin{definition}[Continuous]
    A function $f:A\to\rr$ is continuous if it is continuous at every $c\in A$.
\end{definition}

\section{Continuous Functions on Compact Sets}

\begin{theorem}[Continuous Functions on Compact Sets]{
    Suppose $C\subset\rr$ is compact and $f:C\to \rr$ is continuous.
    Then $f(C)$ is compact.
}
