...
\begin{theorem}[Complete Exponential Theorem]{
    Define $S\coloneqq\{x\in\rr : x^n < c\}$, $S$ is bounded above, then set $\alpha \coloneqq \sup(S)$ and $\alpha^n = c$.
}
    Suppose $\alpha^n < c$.
...
\begin{theorem}[Nested Interval Theorem]{
    Suppose $[a_1,b_1], [a_2,b_2],\ldots$ is a sequence of closed bounded intervals in $\rr$, which is nested.
    \[
...
\begin{remark}
    ...
    \begin{displayquote}
        If $[a_1,b_1], [a_2,b_2],\ldots$ is a sequence of closed bounded intervals in $\rr$ is nested, then $\bigcap_{i=1}^{\infty} [a_i,b_i]\neq\varnothing$.
    \end{displayquote}
...
\begin{theorem}{
    ...
    Suppose $\rr$ is countable, so $\rr=\{x_1,x_2,x_3,\ldots\}$.
